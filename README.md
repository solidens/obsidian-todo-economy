...
Плашка в конце строки хранит оценку в минутах, сложность, приоритет, срок и
идентификатор. Её можно править руками, а можно не трогать. Написал голую
галочку без плашки — плагин усыновит её со значениями по умолчанию и допишет
плашку сам.
...
**Панели нужен моноширинный шрифт.** ASCII-сетка держится на том, что все
символы одной ширины. Плагин проверяет это при открытии панели и говорит прямо,
если текущий шрифт рисует псевдографику не в одну ячейку. Лигатуры отключены
принудительно — иначе `--` склеивается в один глиф и рамка едет.
...
```bash
npm install
npm test      # 105 проверок ядра, без сети и без Obsidian
